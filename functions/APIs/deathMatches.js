...
exports.getAllDeathMatches = (request, response) => {
    db
...
exports.postOneDeathMatch = (request, response) => {
    ...
    db
        ...
        .catch((err) => {
            response.status(500).json({ error: 'Something went wrong creating deathatch' });
            console.error(err);
            return err;
        });
};
...
exports.voteOnDeathhMatch = (request, response) => {
    if (request.body.recipeId || request.body.createdAt) {
        response.status(403).json({ message: 'Not allowed to edit' });
    }
    let document = db.collection('deathMatches').doc(`${request.params.recipeId}`);
...
exports.addRecipeToDeathMatch = (request, response) => {
    if (request.body.recipeId || request.body.createdAt) {
        response.status(403).json({ message: 'Not allowed to edit' });
    }
    let document = db.collection('deathMatches').doc(`${request.params.recipeId}`);
